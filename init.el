...
;; Time-stamp: <2026-07-08T23:01:44+09:00>
...
(leaf whitespace
  ...
  :custom (;; `fill-column' を参照する
           ...
           (whitespace-display-mappings . '(;; LF (U+000A) → "↵" DOWNWARDS ARROW WITH CORNER LEFTWARDS (U+21B5)
                                            (newline-mark ?\u000A [?\u21B5 ?\n])
                                            ;; IDEOGRAPHIC SPACE (U+3000) → "￩￫" HALFWIDTH LEFTWARDS ARROW (U+FFE9), HALFWIDTH RIGHTWARDS ARROW (U+FFEB)
                                            (space-mark ?\u3000 [?\uFFE9 ?\uFFEB])
                                            ;; NO-BREAK SPACE (U+00A0) → "¤" CURRENCY SIGN (U+00A4)
                                            (space-mark ?\u00A0 [?\u00A4])
                                            ;; TAB (U+0009) → "»" RIGHT-POINTING DOUBLE ANGLE QUOTATION MARK (U+00BB)
                                            (tab-mark ?\u0009 [?\u00BB ?\t]))))
  :init
...
(leaf arto
  ...
  :vc (:url "https://github.com/arto-app/arto.el.git" :rev :newest)
...
(leaf dired+
  :after dired
  :vc (:url "https://github.com/emacsmirror/dired-plus.git" :main-file "dired+.el" :rev :newest)
  :require t
...
(leaf ghostel
  :unless (member system-type '(cygwin windows-nt ms-dos))
  :ensure t
  :bind (("C-S-t C-S-t" . ghostel)
         ("C-t C-t" . ghostel-project)
         (:ghostel-mode-map
          ("C-c M-d" . ignore)))
